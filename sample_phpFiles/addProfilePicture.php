...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = "INSERT INTO picture (username, date, img_data)
            VALUES (username = :username, date = :date, img_data = :img_data)";
    $sql_get_pass = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
    $sql_get_pass->execute(array(':username' => $username, ':date' => $date, ':img_data' => $picture));
